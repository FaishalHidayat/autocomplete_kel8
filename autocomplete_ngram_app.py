...
class Dataset(object):
    def __init__(self, sample_ratio=1.0):
        ...
        # Periksa apakah kolom 'review' ada dan tidak kosong
        if 'review' in dataset.columns and not dataset['review'].empty:
            self.lines = list(dataset['review'])
        else:
            print("Kolom 'review' kosong atau tidak ada dalam dataset.")

        self.lines = list(filter(lambda line: len(line) > 0, self.lines))
        print("Total number of examples : ", len(self.lines))

        self.lines = random.sample(self.lines, int(sample_ratio * len(self.lines)))
